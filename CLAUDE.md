...
- `services/` — domain logic that is legitimately frontend-side:
  ...
  - `services/languages/` — three Monaco languages, one per on-disk dialect. `dispatch.ts` maps path → language id (it lives at the top level, not inside a language module, because it is cross-language); `theme.ts` is the **workbench-wide** theme (`defineTheme` once, applied as the editor's `theme` option — token names are shared across all three); `dialect-config.ts` is the `.dstd`/`.dmsg` shared `LanguageConfiguration`.
    - `ecl/` — the full service: Monaco providers (completion, hover, definition, references, signature help, document symbols), `tokenizer` for highlighting, `static-diagnostics` + `toolchain-diagnostics` (thecl output → Monaco markers), and `semantic-loader`/`vocabulary`/`dynamic-vocabulary` which load **eclmap semantic data** to drive the providers. `register.ts` wires it all into Monaco.
    - `std/` — highlighting + jump navigation (definition + code lens). `timeline.ts` owns the timeline semantics and exports `timelineKeywords()`, which the tokenizer reuses so the "special instruction" list has one home.
    - `msg/` — highlighting only. MSG has no jump instruction, so there is nothing to navigate to.

    Neither STD nor MSG needs a **dynamic** vocabulary the way ECL does: the translators write resolvable opcodes as names and unresolvable ones as `ins_N`, so `ins_\d+` alone distinguishes them and the mapfile never has to reach the frontend.

    Monarch gotcha worth knowing before editing a tokenizer: a rule whose regex source **starts with `^`** is flagged `matchOnlyAtLineStart` and is only tried at column 0 (`monarchCompile.js` / `monarchLexer.js`). That is what keeps `.dstd`'s `# THTK-Studio` header rule from swallowing the ARGB colour literals (`#b0d08000`) that appear mid-line. `tests/unit/dialectHighlighting.spec.ts` runs the **real** Monarch tokenizer headless (see `tests/helpers/monarch.ts`) rather than asserting on regexes.
