...
### 待验收：文件树滚动与状态栏移除（2026-08-04）

1. 侧栏文件树在项目文件多于一屏时可用滚轮滚动；文件名过长时可横向拖动查看。
2. 底部蓝色状态栏已消失，终端多出约 24px；外部改动文件时仍能看到重载提示
   （已从状态栏改走 toast）。

### 待验收：MSG 编码（2026-08-05）

1. 项目设为 `shift-jis` 时打包含简体汉字的 `.dmsg`，**打包失败**并指出是第几行
   第几列的哪个字，且建议改用 GBK；原 `.msg` 不被覆盖。
2. 同一份改成 `gbk` 后打包成功。
3. 解包原版日文 `.msg`（`shift-jis`）文本可读，且**不弹**乱码提示。
4. 故意用 `shift-jis` 解一个 GBK 的 `.msg`，输出面板出现「疑似编码不匹配」提示。
5. `.msg` 面板上的编码下拉可单次覆盖项目设置；解包用 shift-jis、打包用 gbk
   的一次往返能走通。

> ⚠️ 第 2、5 项能否在**游戏里**正常显示，取决于游戏侧是否已做汉化适配。本工具
> 只保证字节按所选编码正确写入。见下方「未确证」。
